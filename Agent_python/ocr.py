...
def grade_homework_func(image_url: str) -> str:
    ...
    try:
            ...
            "messages": [
                {
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": "请识别图像中的文本内容并进行批改，给出详细的批改结果和建议。"},
                    ],
                }
            ],
            "stream": False
        ...
        result = completion.choices[0].message.content if completion.choices[0].message.content else "未能获取到批改结果"
        
        return result
        
    except Exception as e:
        logger.error(f"作业批改过程中出现错误: {str(e)}", exc_info=True)
